...
import { RegisteredServer, ServerStatus } from '../models/Server';
import {
...
export class LoadBalancerService implements ILoadBalancerService {
  ...
  async selectServer(
    ...
    let selectedServer: RegisteredServer | null = null;
   ...
   */
  async setServerWeights(groupId: string, weights: ServerWeight[]): Promise<void> {
    // Validate weights
   ...
   */
  async getServerWeights(groupId: string): Promise<ServerWeight[]> {
   ...
   */
  async recordSuccess(serverId: string): Promise<void> {
   ...
   */
  async recordFailure(serverId: string): Promise<void> {
    const status = this.getOrCreateCircuitBreakerStatus(serverId);
    status.failureCount++;
   ...
   */
  async getCircuitBreakerStatus(serverId: string): Promise<CircuitBreakerStatus> {
    const status = this.getOrCreateCircuitBreakerStatus(serverId);
    
    // Check if we should transition from OPEN to HALF_OPEN
    if (status.state === CircuitBreakerState.OPEN && status.nextAttemptTime) {
   ...
   */
  async resetCircuitBreaker(serverId: string): Promise<void> {
    const status = this.getOrCreateCircuitBreakerStatus(serverId);
    status.state = CircuitBreakerState.CLOSED;
    status.failureCount = 0;
    status.successCount = 0;
   ...
   */
  private async filterAvailableServers(servers: RegisteredServer[]): Promise<RegisteredServer[]> {
    const availableServers: RegisteredServer[] = [];
  ...
  private async selectRoundRobin(
    ...
    const key = context?.groupId || 'default';
  ...
  private async selectWeighted(
    ...
    const weightedServers: { server: RegisteredServer; weight: number }[] = [];
    ...
    return weightedServers[0]?.server || null;
  }

  /**
   * Select server with least connections
   */
  private async selectLeastConnections(servers: RegisteredServer[]): Promise<RegisteredServer | null> {
    if (servers.length === 0) return null;
   ...
   */
  private selectRandom(servers: RegisteredServer[]): RegisteredServer | null {
    if (servers.length === 0) return null;
   ...
   */
  private getOrCreateCircuitBreakerStatus(serverId: string): CircuitBreakerStatus {
