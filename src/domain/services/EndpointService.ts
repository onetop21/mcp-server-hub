...
import { IEndpointService } from './IEndpointService';
...
export class EndpointService implements IEndpointService {
  ...
  constructor(
    ...
    baseUrl: string = 'https://hub.mcp.local'
  ) {
   ...
   */
  async createDefaultEndpoint(userId: string, apiKeyId: string): Promise<Endpoint> {
   ...
   */
  async createGroupEndpoint(userId: string, groupId: string, apiKeyId: string): Promise<Endpoint> {
    // Validate API key belongs to user
    await this.validateApiKeyOwnership(userId, apiKeyId);
   ...
   */
  async getEndpointsByUser(userId: string): Promise<Endpoint[]> {
   ...
   */
  async updateLastAccessed(endpointId: string): Promise<void> {
    const endpoint = await this.endpointRepository.findEndpointById(endpointId);
    if (!endpoint) {
      throw new EndpointError('Endpoint not found', 'ENDPOINT_NOT_FOUND');
    }
   ...
   */
  async validateEndpointAccess(endpointId: string, apiKey: string): Promise<boolean> {
    try {
      // Get endpoint
      const endpoint = await this.endpointRepository.findEndpointById(endpointId);
      if (!endpoint) {
   ...
   */
  async getEndpointsWithGroupInfo(userId: string): Promise<Array<Endpoint & { groupName?: string }>> {
   ...
   */
  async getDefaultEndpoint(userId: string): Promise<Endpoint | null> {
   ...
   */
  async regenerateEndpointUrls(endpointId: string): Promise<Endpoint> {
    const endpoint = await this.endpointRepository.findEndpointById(endpointId);
    if (!endpoint) {
      throw new EndpointError('Endpoint not found', 'ENDPOINT_NOT_FOUND');
    }
   ...
   */
  private async validateApiKeyOwnership(userId: string, apiKeyId: string): Promise<void> {
   ...
   */
  private async ensureUrlsAreUnique(
    url: string, 
    sseUrl: string, 
    httpUrl: string, 
...
export const ENDPOINT_SERVICE = Symbol.for('EndpointService');