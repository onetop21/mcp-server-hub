import { ServerGroup, GroupConfig, RoutingRule } from '../../domain/models/Endpoint';
import { BaseRepository } from '../database/BaseRepository';
import { DatabaseConnection } from '../database/connection';

/**
 * Database representation of ServerGroup
 */
interface ServerGroupRow {
  ...
  routing_rules: any; // JSONB
  created_at: Date;
  updated_at: Date;
}

/**
 * Database representation of ServerGroupMember
 */
interface ServerGroupMemberRow {
  id: string;
  server_id: string;
  group_id: string;
  created_at: Date;
 ...
 */
export class ServerGroupRepository extends BaseRepository<ServerGroupRow> {
   ...
   */
  private toDomainModel(row: ServerGroupRow, serverIds: string[] = []): ServerGroup {
    return {
      id: row.id,
      userId: row.user_id,
      name: row.name,
      ...
      serverIds,
      routingRules: (row.routing_rules as RoutingRule[]) || [],
   ...
   */
  private toDbRow(group: Partial<ServerGroup>): Partial<ServerGroupRow> {
    const row: Partial<ServerGroupRow> = {};
    
    if (group.userId !== undefined) row.user_id = group.userId;
    if (group.name !== undefined) row.name = group.name;
    if (group.description !== undefined) row.description = group.description;
    if (group.routingRules !== undefined) row.routing_rules = group.routingRules;
    if (group.createdAt !== undefined) row.created_at = group.createdAt;
    if (group.updatedAt !== undefined) row.updated_at = group.updatedAt;
   ...
   */
  async findByUserId(userId: string): Promise<ServerGroup[]> {
   ...
   */
  async findByUserIdAndName(userId: string, name: string): Promise<ServerGroup | null> {
   ...
   * Update group
   */
  async updateGroup(id: string, updates: Partial<ServerGroup>): Promise<ServerGroup | null> {
   ...
   */
  async addServersToGroup(groupId: string, serverIds: string[]): Promise<void> {
   ...
   */
  async removeServersFromGroup(groupId: string, serverIds: string[]): Promise<void> {
    if (serverIds.length === 0) return;
   ...
   */
  async addServerToGroup(serverId: string, groupId: string): Promise<void> {
    const query = `
      INSERT INTO server_group_members (server_id, group_id) 
      VALUES ($1, $2) 
      ON CONFLICT (server_id, group_id) DO NOTHING
   ...
   */
  async getServerIdsByGroupId(groupId: string): Promise<string[]> {
    const query = 'SELECT server_id FROM server_group_members WHERE group_id = $1 ORDER BY created_at';
    const result = await this.db.query<{ server_id: string }>(query, [groupId]);
   ...
   */
  async getGroupsByServerId(serverId: string): Promise<ServerGroup[]> {
    const query = `
      SELECT sg.* 
    ...
    const result = await this.db.query<ServerGroupRow>(query, [serverId]);
   ...
   */
  async updateRoutingRules(groupId: string, routingRules: RoutingRule[]): Promise<void> {
    const query = `
  ...
  async getStatistics(): Promise<{
    ...
    averageServersPerGroup: number;
  }> {
    const totalGroups = await this.count();
    
    const membershipQuery = 'SELECT COUNT(*) as count FROM server_group_members';
    const membershipResult = await this.db.query<{ count: string }>(membershipQuery);
    const totalMemberships = parseInt(membershipResult.rows[0]?.count || '0', 10);
