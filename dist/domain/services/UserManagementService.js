...
exports.USER_MANAGEMENT_SERVICE = exports.UserManagementService = void 0;
...
class UserManagementService {
    constructor(userRepository, apiKeyRepository, passwordHasher, tokenGenerator, rateLimiter, usageTrackingService, permissionService) {
    ...
    async createUser(userData) {
        ...
        const user = await this.userRepository.createUser({
            ...
            passwordHash,
            subscription: userData.subscription || User_1.SubscriptionTier.FREE,
        });
        return user;
    }
    /**
    ...
    async authenticateUser(credentials) {
        ...
        // Verify password
        const isValidPassword = await this.passwordHasher.verify(credentials.password, user.passwordHash);
        if (!isValidPassword) {
            throw new Error('Invalid credentials');
        }
        // Update last login
     ...
     */
    async generateApiKey(userId, permissions, name, expiresIn) {
        // Verify user exists
    ...
    async checkRateLimit(apiKey) {
        ...
        if (this.usageTrackingService) {
            return await this.usageTrackingService.checkRateLimit(apiKey, validation.rateLimit.requestsPerHour, validation.rateLimit.requestsPerDay);
        }
        else {
            // Check rate limit with basic rate limiter
            const status = await this.rateLimiter.checkRateLimit(apiKey, validation.rateLimit.requestsPerHour, validation.rateLimit.requestsPerDay);
     ...
     */
    async recordRequest(apiKey, endpoint, toolName) {
        if (this.usageTrackingService) {
     ...
     */
    async checkPermission(apiKey, resource, action) {
        if (!this.permissionService) {
     ...
     */
    async validateUserCreateRequest(userData) {
        // Email validation
     ...
     */
    getDefaultRateLimit(subscription) {
        switch (subscription) {
            case User_1.SubscriptionTier.FREE:
                return {
                    requestsPerHour: 100,
                    requestsPerDay: 1000,
                    maxServers: 3,
                };
            case User_1.SubscriptionTier.BASIC:
                return {
                    requestsPerHour: 500,
                    requestsPerDay: 10000,
                    maxServers: 10,
                };
            case User_1.SubscriptionTier.PREMIUM:
                return {
                    requestsPerHour: 2000,
                    requestsPerDay: 50000,
                    maxServers: 50,
                };
            case User_1.SubscriptionTier.ENTERPRISE:
                return {
...
exports.USER_MANAGEMENT_SERVICE = Symbol.for('UserManagementService');
