...
const RedisRateLimiter_1 = require("../../infrastructure/utils/RedisRateLimiter");
class UsageTrackingService {
    constructor(redisConnection) {
        this.redisConnection = redisConnection;
        this.rateLimiter = new RedisRateLimiter_1.RedisRateLimiter(redisConnection);
    }
    /**
     ...
     */
    async getDetailedUsageStats(apiKey) {
        const client = this.redisConnection.getClient();
        ...
        const totalRequests = parseInt(results[0] || '0', 10);
        const lastRequestTimestamp = results[1];
        ...
        return Object.assign(Object.assign({}, basicStats), { totalRequests,
            requestsByEndpoint,
            requestsByTool,
            lastRequestAt });
    }
     ...
     */
    async checkServerLimit(userId, maxServers) {
     ...
     */
    async recordServerRegistration(userId) {
        const client = this.redisConnection.getClient();
     ...
     */
    async recordServerRemoval(userId) {
        const client = this.redisConnection.getClient();
     ...
     */
    async getCountersByPattern(pattern) {
        const client = this.redisConnection.getClient();
        const keys = await client.keys(pattern);
     ...
     */
    async getDailyRequestsForUser(userId, days) {
        const client = this.redisConnection.getClient();
        const result = new Map();
        const now = new Date();
